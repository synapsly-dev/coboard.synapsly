    ...
    >
      ...
      <View className="task-card__footer">
        <View className="task-card__meta">
          {task.projectName && <Text>▣ {task.projectName}</Text>}
          <Text>◷ {stagePrefix} {shortDate(stageDate ?? task.createdAt)}</Text>
          {task.dueDate && <Text className="task-card__due">◷ DDL {shortDate(task.dueDate)}</Text>}
          <Text>{task.claimants.length}/{task.maxClaimants ?? '∞'} 人</Text>
        </View>
