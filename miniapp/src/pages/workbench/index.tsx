...
function WorkSection({ icon, title, count, children }: { icon: string; title: string; count?: number; children: React.ReactNode }): JSX.Element {
  return <View className="work-section"><View className="work-section__head"><Text className="work-section__icon">{icon}</Text><Text className="work-section__title">{title}</Text>{count != null && count > 0 && <Badge>{count}</Badge>}</View>{children}</View>;
}
