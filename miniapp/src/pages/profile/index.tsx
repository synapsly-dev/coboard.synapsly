...
function ProfilePage(): JSX.Element {
  ...
  if (!user.data) return <View className="page"><PageHeader title="登录 Coboard" description="使用与网页版相同的 Syna ID，账号、角色和项目权限完全一致。" />{authConfig.data?.synapslyEnabled && <ActionButton onClick={() => void Taro.navigateTo({ url: '/pages/auth-login/index' })}>使用 Syna ID 登录</ActionButton>}{(isLocalDevelopment || authConfig.data?.devLogin) && <Card className="stack"><Text className="title">本地开发登录</Text><Input className="field__control" value={email} onInput={(event) => setEmail(event.detail.value)} placeholder="邮箱" /><ActionButton loading={devLogin.isPending} onClick={() => devLogin.mutate()}>开发登录</ActionButton></Card>}</View>;
